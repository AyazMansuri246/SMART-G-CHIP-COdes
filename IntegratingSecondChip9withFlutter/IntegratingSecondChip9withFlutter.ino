...
void onReceive(const esp_now_recv_info* info, const uint8_t* incomingData, int len) {
  ...
  if (packetType == PKT_CONTROL) {
    ...
    // }
    // else 
    if (strcmp(msg.cmd, "START_REC") == 0) {
      startVideo();               // starts camera + audio flag
      sendStatus("REC_STARTED", mac);
    }
    else if (strcmp(msg.cmd, "STOP_REC") == 0) {
      stopVideo();                // stops producers
      sendStatus("REC_STOPPED", mac);
    }
  }

  // ---------- AUDIO (RING BUFFER ONLY WITH DUPLICATE CHECK) ----------
  else if (packetType == PKT_AUDIO) {

    if (!audioRecording) return;
    if (payloadLen < 2) return;  // must contain sequence number
    ...
    // ---- real audio data starts after seq ----
...
void setup() {
  ...
  // /download?folder=Video_001&type=video
  // /download?folder=Video_001&type=audio
  asyncServer.on("/download", HTTP_GET, [](AsyncWebServerRequest *req) {
    ...
    if (!folder.startsWith("Video")) {
      req->send(400, "text/plain", "Invalid folder name");
      return;
    }
    ...
    String tmpPath  = "/Videos/" + folder + "/video.tmp";

    if (sdMutex) xSemaphoreTake(sdMutex, portMAX_DELAY);
    bool exists    = SD_MMC.exists(fullPath.c_str());
    bool tmpExists = SD_MMC.exists(tmpPath.c_str());
    if (sdMutex) xSemaphoreGive(sdMutex);
...
void loop() {
  ...
  if (doCapture) { doCapture = false; captureImage(); wsClient.sendTXT("IMG_CAPTURED"); }
}
