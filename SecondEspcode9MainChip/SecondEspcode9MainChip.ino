...
void sendCmd(const char* cmd) {
  sendToCAM(cmd);   // ✅ only goes to CAM, not phone
}
...
void handleLongPress() {
  if (isRecording) {
    ...
    sendToCAM("STOP_REC");
    isRecording = false;
    videoRecording = false;  // ← stop audio to CAM
    totalSent = 0;
  } else {
...
void loop() {
  ...
  if (liveMode && activeClientId != 0) {
    AsyncWebSocketClient *phone = ws.client(activeClientId);
    if (phone && phone->canSend()) {
      phone->binary(chunk, 512);
    }
  }

  // send to CAM if video recording
  if (videoRecording && camClientId != 0) {
    AsyncWebSocketClient *cam = ws.client(camClientId);
    if (cam && cam->canSend()) {
      cam->binary(chunk, 512);
    }
  }
